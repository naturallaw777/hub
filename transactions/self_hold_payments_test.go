...
func TestSelfHoldPaymentSettled(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		result, err := transactionsService.SendPaymentSync(transaction.PaymentRequest, nil, nil, svc.LNClient, nil, nil, false)
...
func TestWrappedInvoice(t *testing.T) {
	...
	*/
	...
	go func() {
		defer wg.Done()
		result, err := transactionsService.SendPaymentSync(bobWrappedInvoice.PaymentRequest, nil, nil, svc.LNClient, &aliceApp.ID, nil, false)
		assert.NoError(t, err)
		require.NotNil(t, result)
		assert.Equal(t, constants.TRANSACTION_STATE_SETTLED, result.State)
	}()
	...
	result, err := transactionsService.SendPaymentSync(charlieInvoice.PaymentRequest, nil, nil, svc.LNClient, &bobApp.ID, nil, false)
	assert.NoError(t, err)
	require.NotNil(t, result)
	assert.Equal(t, constants.TRANSACTION_STATE_SETTLED, result.State)
